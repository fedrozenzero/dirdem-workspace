...
module.exports = function compileCProject() {
  ipcMain.on('compileCProject', (event, arg) => {
    ...
    try {
      ...
      event.reply('compileCProjectFailed', error);
      return;
    }
    ...
    findCFiles(projectPath).then(filePathToBuild => {
      compileCFiles(filePathToBuild, projectPath, microName, outputFolderPath).then(compileOutput => {
        ...
        event.reply('compileCProjectCompleted', response);
      }, rej1 => {
        event.reply('compileCProjectFailed', rej1);
      });
    }, rej2 => {
      event.reply('compileCProjectFailed', rej2);
    })
  })
}
